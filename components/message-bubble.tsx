  ...
  return (
    ...
    >
      ...
      <div className={cn("flex flex-col", isUser ? "items-end" : "items-start")}>
        ...
        {/* Attachments */}
        {attachments.length > 0 && <div className="mt-1 space-y-1 max-w-[85%]">{attachments}</div>}

        {/* Status Row */}
        <div className={cn("flex items-center gap-1 mt-1 text-xs", isUser ? "justify-end" : "justify-start")}>
          {/* Timestamp */}
          <span className="text-slate-400">{formatTime(timestamp)}</span>
...
export function MessageAttachment({ type, url, filename, filesize }: AttachmentProps) {
  return (
